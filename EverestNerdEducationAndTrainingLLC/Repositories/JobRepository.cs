...
namespace EverestNerdEducationAndTrainingLLC.Repositories
{
    public class JobRepository : IJobRepository
    {
        ...
        public int EditJob(Jobs jobs)
        {
            try
            {
                string query = "Update Jobs Set Title = @Title, Description = @Description, Requirement = @Requirement, Salary = @Salary, Country = @Country, City = @City, Tags = @Tags, State = @State where Id = @Id Select @Id";
        ...
        public int AddJobs(Jobs jobs)
        {
            try
            {   
                string query = "Insert into Jobs (Title, Description, Requirement, Salary, Country, City, Tags, State) Values(@Title, @Description, @Requirement, @Salary, @Country, @City, @Tags, @State) SELECT SCOPE_IDENTITY() AS [SCOPE_IDENTITY]";
                var param = new Dictionary<string, object>();
                param.Add("@Title", jobs.Title);
                param.Add("@Description", jobs.Description);
                param.Add("@Requirement", jobs.Requirement);
        ...
        {
            Serilog.Log.Information("GetAllContactList Method Called");
            try
            {
                string query = "Select * from Contact;";
                var _parameter = new Dictionary<string, object>();
                List<Contact> result = conn.Query<Contact>(query, param: _parameter, commandType: CommandType.Text).ToList();
                return result;
            }
            catch (Exception ex)
            {
